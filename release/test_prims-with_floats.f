...
: test_float
    ...
    1.99 f1+
        2.99 = IF ." float-increment test passed" LF passed
        ELSE ." float-increment test failed" LF failed
        THEN FLUSH
    \\ ----------------------------------
    \\ test: float-add
    1.50 1.55 f+
        3.05 = IF ." float-add test passed" LF passed
        ELSE ." float-add test failed" LF failed
        THEN FLUSH
    \\ ----------------------------------
    \\ test: float-subtract
    3.978 3.008 f-
        0.970 = IF ." float-subtract test passed" LF passed
        ELSE ." float-subtract test failed" LF failed
        THEN FLUSH
    \\ ----------------------------------
    \\ test: float-multiply
    1.50 2.05 f*
        3.75 = IF ." float-multiply test passed" LF passed
        ELSE ." float-multiply test failed" LF failed
        THEN FLUSH
    \\ ----------------------------------
    \\ test: float-divide
    4.98 1.69 f/
        2.94674556213 = IF ." float-divide test passed" LF passed
        ELSE ." float-divide test failed" LF failed
        THEN FLUSH
    \\ ----------------------------------
    \\ test: float-multiply-divide
    2.2 2.2 4.0 f*/
        1.21 = IF ." float-multiply-divide test passed" LF passed
        ELSE ." float-multiply-divide test failed" LF failed
        THEN FLUSH
